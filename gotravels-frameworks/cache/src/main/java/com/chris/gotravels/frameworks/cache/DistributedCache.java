package com.chris.gotravels.frameworks.cache;

import com.chris.gotravels.frameworks.cache.core.CacheGetFilter;
import com.chris.gotravels.frameworks.cache.core.CacheGetIfAbsent;
...
public interface DistributedCache extends Cache{
    ...
    /**
     * 以一种"安全"的方式获取缓存，如查询结果为空，调用 {@link CacheLoader} 加载缓存
     * 通过此方式防止程序中可能出现的：缓存穿透、缓存击穿以及缓存雪崩场景
     ...
     * 以一种"安全"的方式获取缓存，如查询结果为空，调用 {@link CacheLoader} 加载缓存
     * 通过此方式防止程序中可能出现的：缓存穿透、缓存击穿以及缓存雪崩场景
     * 需要客户端传递布隆过滤器，并通过 {@link CacheGetFilter} 解决布隆过滤器无法删除问题，适用于被外部直接调用的接口
     */
    <T> T safeGet(
            @NotBlank String key,
            Class<T> clazz,
     ...
     */
    void put(
            @NotBlank String key, Object value,
            long timeout
    );

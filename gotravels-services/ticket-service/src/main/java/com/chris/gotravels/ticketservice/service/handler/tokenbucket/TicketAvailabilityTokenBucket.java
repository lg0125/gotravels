package com.chris.gotravels.ticketservice.service.handler.tokenbucket;

import cn.hutool.core.util.StrUtil;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson2.JSONArray;
import com.alibaba.fastjson2.JSONObject;
import com.chris.gotravels.frameworks.base.Singleton;
import com.chris.gotravels.frameworks.cache.DistributedCache;
import com.chris.gotravels.frameworks.common.toolkit.Assert;
...
import com.chris.gotravels.ticketservice.dto.domain.SeatTypeCountDTO;
...
public class TicketAvailabilityTokenBucket {
    ...
    public boolean takeTokenFromBucket(PurchaseTicketReqDTO requestParam) {
        ...
        if (!hasKey) {
            RLock lock = redissonClient.getLock(
                    String.format(
                            LOCK_TICKET_AVAILABILITY_TOKEN_BUCKET,
                            requestParam.getTrainId()
                    )
            );
            ...
            try {
                ...
                if (!hasKeyTwo) {
                    ...
                    for (RouteDTO each : routeDTOList) {
                        List<SeatTypeCountDTO> seatTypeCountDTOList = seatMapper.listSeatTypeCount(
                        ...
                        for (SeatTypeCountDTO eachSeatTypeCountDTO : seatTypeCountDTOList) {
                            String buildCacheKey = StrUtil.join(
                                    "_",
                                    each.getStartStation(),
                                    each.getEndStation(),
        ...
        DefaultRedisScript<Long> actual = Singleton.get(
        ...
        String luaScriptKey = StrUtil.join(
                "_",
                requestParam.getDeparture(),
                requestParam.getArrival()
        );
    ...
    public void rollbackInBucket(TicketOrderDetailRespDTO requestParam) {
        ...
        Map<Integer, Long> seatTypeCountMap = passengerDetails.stream()
                .collect(
                        Collectors.groupingBy(
                                TicketOrderPassengerDetailRespDTO::getSeatType,
                                Collectors.counting()
                        )
                );

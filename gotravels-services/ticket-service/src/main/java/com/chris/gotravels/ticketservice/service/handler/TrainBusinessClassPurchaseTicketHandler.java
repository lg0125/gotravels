...
import com.chris.gotravels.ticketservice.service.handler.base.BitMapCheckSeat;
import com.chris.gotravels.ticketservice.service.handler.base.BitMapCheckSeatStatusFactory;
import com.chris.gotravels.ticketservice.toolkit.CarriageVacantSeatCalculateUtil;
...
import static com.chris.gotravels.ticketservice.service.handler.base.BitMapCheckSeatStatusFactory.TRAIN_BUSINESS;
...
public class TrainBusinessClassPurchaseTicketHandler extends AbstractTrainPurchaseTicketTemplate {
    ...
    protected List<TrainPurchaseTicketRespDTO> selectSeats(SelectSeatDTO requestParam) {
        ...
        if (passengerSeatDetails.size() < 3) {
            if (CollUtil.isNotEmpty(requestParam.getRequestParam().getChooseSeats())) {
                ...
                Pair<List<TrainPurchaseTicketRespDTO>, Boolean> actualSeatPair = findMatchSeats(
                        requestParam,
                        trainCarriageList,
    ...
    private Pair<List<TrainPurchaseTicketRespDTO>, Boolean> findMatchSeats(
        ...
        for (int i = 0; i < trainStationCarriageRemainingTicket.size(); i++) {
            ...
            boolean isExists = instance.checkChooseSeat(
        ...
        for (int i = 0; i < trainStationCarriageRemainingTicket.size(); i++) {
            ...
            if (actualSelects.size() == splitPassengerSeatDetails.size()) {
                ...
                for (int j = 0; j < actualSelects.size(); j++) {
                    if (j == 0)
                        actualSelect = mergeArrays(
                                actualSelects.get(j),
                                actualSelects.get(j + 1)
                        );

...
import static com.chris.gotravels.ticketservice.common.constant.RedisKeyConstant.LOCK_QUERY_ALL_REGION_LIST;
import static com.chris.gotravels.ticketservice.common.constant.RedisKeyConstant.QUERY_ALL_REGION_LIST;
...
public class TrainTicketQueryParamVerifyChainFilter implements TrainTicketQueryChainFilter<TicketPageQueryReqDTO> {
    ...
    public void handler(TicketPageQueryReqDTO requestParam) {
        ...
        // actualExistList就算为空，也会返回数据，所以通过 filter 判断对象是否为空
        ...
        // FLAG=false代表有可能缓存没有数据，但数据库可能有，此时向下查询数据库
        // 为了避免缓存击穿，所以这里是使用分布式锁
        ...
        try {
            ...
            // 再查询一次，查看是否存在
            // 如果加载后还是为空，那么直接抛出异常
            emptyCount = regionValueMap.keySet().stream().filter(
                        each -> StrUtil.equalsAny(
                                each.toString(),
